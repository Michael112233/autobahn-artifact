...
def process_logs(faults=0, save_to_file=True):
    ...
    try:
        ...
        Print.warn(f'Error processing logs: AssertionError - {e}')
        Print.warn('This may indicate log data inconsistencies (e.g., received transactions that were not sent).')
        import traceback
        ...
        Print.warn(f'Error processing logs: {error_msg}')
        import traceback
        Print.warn('Traceback:')
        Print.warn(traceback.format_exc())
        return False
